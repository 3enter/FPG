...
namespace FPG.Class
{
    public class FPGrowth<T> where T : IComparable<T>
    {
        ...
        {
            var ssss = MinePatterns(database, domain, (itemset) => { return minSupport; });
        ...
        public ItemSets<T> MinePatterns(IEnumerable<Transaction<T>> database, IList<T> domain, GetMinSupportHandle getMinItemSetSupport)
        {
            ...
            for (int i = 0; i < domain.Count; ++i)
            {
                var xxx = getMinItemSetSupport(new ItemSet<T>() { domain[i] });
